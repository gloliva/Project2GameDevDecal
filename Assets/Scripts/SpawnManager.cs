...
public class SpawnManager : MonoBehaviour
{
    ...
    [SerializeField]
    private float enemyRateIncreaseValue;
    [SerializeField]
    private float powerupRateIncreaseValue;
    #endregion
    ...
    private void Update()
    {
        ...
        if (increaseSpawnRate <= 0.0f)
        {
            for (int i = 0; i < m_EnemyTypes.Length; i++)
            {
                if (m_EnemyTypes[i].EnemyPrefab.CompareTag("Melee Enemy") || m_EnemyTypes[i].EnemyPrefab.CompareTag("Ranged Enemy"))
                {
                    m_EnemyTypes[i].SpawnRateSet = enemyRateIncreaseValue;
                }
                else
                {
                    m_EnemyTypes[i].SpawnRateSet = powerupRateIncreaseValue;
                }
            }
            increaseSpawnRate = 5.0f;
        } else
        {
            increaseSpawnRate -= Time.deltaTime;
        }
    }
    #endregion
}
